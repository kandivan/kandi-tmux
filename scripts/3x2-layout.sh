#!/bin/bash
# Rearrange current window's panes into a 3-column x 2-row grid
...
if [ "$count" -ne 6 ]; then
  tmux display-message "Need exactly 6 panes for 3x2 layout (have $count)"
  exit 1
fi
...
# Column widths (3 columns, accounting for 2 separator lines)
